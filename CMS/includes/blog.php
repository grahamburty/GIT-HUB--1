...
//blog default			
	...
	for ($n=$start-1  ;  $n>=$end  ;  $n-- ) { 
		...
		if (isset($blog[0]) & $blog[0] != '') {
			...
			if($rewrite) { echo "<h2 class=\"blog-title\"><a href=\"blog-". $blog[0] ."-$clean_url\">" .$blog[3]."</a></h2>\n"; }
			...
			if(preg_match("/^(.*)##more##/U", $blog[4], $m)) {
				    if($rewrite) { echo "<div class=\"blog-content\">" . $m[1] ." <a href=\"blog-". $blog[0] ."-$clean_url\" class=\"read-more\">$lang_blog_more</a></div>\n";}
			...
			if($blog_comments) {
				if($blog[1]) {
				    if($rewrite) { echo  "<div class=\"num_comments\"><a href=\"blog-". $blog[0] ."-$clean_url#comments\">(". $blog[1] .") $lang_blog_num_comment</a></div>\n\n";}
				      else {echo  "<div class=\"num_comments\"><a href=\"?d=". $blog[0] ."#comments\">(". $blog[1] .") $lang_blog_num_comment</a></div>\n"; }
				   } else {
						if($rewrite) { echo "<div class=\"num_comments\"><a href=\"blog-". $blog[0] ."-$clean_url#comments\">$lang_blog_no_comment</a></div>\n\n";}
